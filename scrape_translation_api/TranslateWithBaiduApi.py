...
target_site = "fanyi.baidu.com"
target_path = "/v2transapi"
...
handler = logging.FileHandler('TranslateWithBaiduApi.' + str(int(time.time())) + ".log")
handler.setFormatter(log_format)
log = logging.getLogger("TranslateWithBaiduApi")
...
def construct_body(data):
    return urllib.parse.urlencode(
        {
            "from":source_lanauage, 
            "to":target_language, 
            "query": data, 
            "transtype":"translang", 
            "simple_means_flag": 3}).encode()
...
def parse_response(response):
    ...
    # print(ret)
    lines = []
    for rec in ret["trans_result"]["data"]:
        lines.append(rec["dst"])
    #print(lines)
    return lines
...
def add_line_parts(lines, line_parts):
    if len(line_parts) == 2:
        ...
        lines.append((line_parts[0].strip(), None))
...
if __name__ == "__main__":
    ...
    try:
        ...
        for line in source_file:
            ...
            lines_size_in_bytes += sys.getsizeof(line_parts[0]) + 1
