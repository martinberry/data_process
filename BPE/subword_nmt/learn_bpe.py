...
def get_vocabulary(fobj):
    ...
    for i, line in enumerate(fobj):
        for word in line.split():
            vocab[word] += 1
        if i % 1000000 == 0: print('processed %d lines, len vocab %d' % (i, len(vocab)))
    return vocab
...
def get_pair_statistics(vocab):
    ...
    for i, (word, freq) in enumerate(vocab):
        ...
        for char in word[1:]:
            ...
            prev_char = char
        if i % 100000 == 0:
            print('processed %d vocab items, total %d' % (i, len(vocab)))
